"""hpmcounter5 raw value over time, one plot per seed, all 4 modes overlaid
on each plot.

Reuses the raw hpc csv reading approach from line_fitting_analysis.py
(hex_to_int decode, elapsed = timestamp_ms - timestamp_ms[0]).
"""
...
PLOT_DIR = os.path.join(SCRIPT_DIR, "plots", "hpmcounter5_per_seed")
COUNTER = "hpmcounter5"
...
def main():
    ...
    for seed_name in seed_names:
        ...
        ax.set_title(f"{seed_name}: {COUNTER} raw value over time, all modes", color=TEXT_PRIMARY, fontsize=12)
        ...
        out_path = os.path.join(PLOT_DIR, f"{seed_name}_{COUNTER}.png")
