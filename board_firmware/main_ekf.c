 ...
 * PRECOMPUTED-TRAJECTORY VERSION
 *
...
#include "ekf_config.h"     /* MUST come before tinyekf.h.
                              ...
                              generate_trajectories.py reads them
                              directly for the offline computation. */
#include "tinyekf.h"
#include "trajectory.h"     /* now provides traj_true[][3] and
                              traj_measured[][3], not a single
                              traj[][3] array */
 ...
 * ONE EKF STEP
 *
 * Takes the measured position directly -- no mode string, no
 * on-device spoof application. spoof_error is computed simply as
 * the distance between the measured and true positions, which are
 * both already given (previously this required apply_spoof() to
 * compute; now it's just the two precomputed arrays).
 * ============================================================ */
...
int main(void)
{
    ...
    /* Run TRAJ_LEN-1 EKF steps -- HPM counters accumulate.
       ...
       precomputed measured/true pair for this step and hand it
       to the filter. Every mode runs this exact same loop. */
