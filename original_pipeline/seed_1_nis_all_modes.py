"""NIS values over time for seed_1: one separate plot per mode
...
def main():
    ...
    for mode in MODES:
        ...
        ax.plot(df["t"], df["nis"], color=MODE_COLOR[mode], linewidth=1.5, label=mode)

        ax.set_title(f"seed_1 {mode} run: NIS over entries", color=TEXT_PRIMARY, fontsize=12)
        ...
        out_path = os.path.join(PLOT_DIR, f"seed_1_nis_{mode}.png")
