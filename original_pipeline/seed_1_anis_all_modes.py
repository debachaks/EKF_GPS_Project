"""ANIS values over time for seed_1: one separate plot per mode
(normal, jump, drift, replay)."""
...
def main():
    ...
    for mode in MODES:
        ...
        ax.plot(df["t"], df["anis"], color=MODE_COLOR[mode], linewidth=1.5, label=mode)

        ax.set_title(f"seed_1 {mode} run: ANIS over entries", color=TEXT_PRIMARY, fontsize=12)
        ax.set_xlabel("t (entry index)", color=TEXT_MUTED)
        ax.set_ylabel("anis", color=TEXT_MUTED)
        ax.legend(frameon=False, labelcolor=TEXT_PRIMARY, fontsize=9)
        style_axes(ax)
        fig.tight_layout()

        out_path = os.path.join(PLOT_DIR, f"seed_1_anis_{mode}.png")
