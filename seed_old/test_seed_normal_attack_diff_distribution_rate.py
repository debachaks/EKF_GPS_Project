"""normal-vs-attack pairwise (same-seed) difference distribution, per counter.

Companion to test_seed_normal_pairwise_diff_distribution_rate.py (which
established the normal-vs-normal noise baseline). This script computes,
for each counter and each attack type, the signed rate difference
(normal - attack) run-wise: for each of the 20 seeds, normal's own rate
trace minus that same seed's jump/drift/replay rate trace, interpolated
onto a shared elapsed-time grid. Across the 20 seeds this gives 20
difference curves per attack type; the plot shows their mean (solid) and
mean +/- 1 std (dotted), for jump, drift, and replay together on one
figure per counter.

Compare against the normal-vs-normal noise band from the companion script:
if an attack's mean difference sticks out beyond where normal-vs-normal
noise sits, that's a difference bigger than ordinary run-to-run noise.

Output: test_seed_normal_attack_diff_plots/<counter>_normal_vs_attack_diff_rate.png
"""
...
def rate_trace(seed_name, mode, counter):
    path = os.path.join(CLEAN_ROOT, seed_name, f"ekf_{mode}_hpc.csv")
    df = pd.read_csv(path)
...
def build_mean_std(seed_names, mode, counter):
    normal_traces = {s: rate_trace(s, "normal", counter) for s in seed_names}
    attack_traces = {s: rate_trace(s, mode, counter) for s in seed_names}
    ...
    diffs = []
    for s in seed_names:
        n_elapsed, n_rate = normal_traces[s]
        a_elapsed, a_rate = attack_traces[s]
        n_interp = np.interp(grid, n_elapsed, n_rate)
        a_interp = np.interp(grid, a_elapsed, a_rate)
...
def plot_counter(counter, seed_names, out_dir):
    ...
    ax.set_title(f"{counter} (rate): normal minus attack, run-wise (mean +/- 1 std)",
                 color=TEXT_PRIMARY, fontsize=12)
    ax.set_xlabel("elapsed time (ms since run start)", color=TEXT_MUTED)
    ax.set_ylabel("normal_rate - attack_rate (events/cycle)", color=TEXT_MUTED)
    ax.legend(frameon=False, labelcolor=TEXT_PRIMARY, fontsize=9)
    style_axes(ax)

    fig.tight_layout()
    out_path = os.path.join(out_dir, f"{counter}_normal_vs_attack_diff_rate.png")
