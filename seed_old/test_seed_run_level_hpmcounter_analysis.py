"""Run-level comparison of hpmcounter columns (post-cleaning) for
test_seed_1..3, using the cleaned files from CLEAN_HPC_TEST_SEED/test_seed_N/.

Same methodology as run_level_hpmcounter_analysis.py /
new_hpm_run_level_hpmcounter_analysis.py: each seed's run is reduced to a
per-run median per counter first (rows within a run are an autocorrelated
time series, not independent samples), then the N=3 normal-run medians are
compared against the N=3 anomaly-run medians per counter via Mann-Whitney U,
with Cliff's delta as the effect size.
"""
...
SUMMARY_OUT = os.path.join(SCRIPT_DIR, "test_seed_run_level_summary.csv")
STATS_OUT = os.path.join(SCRIPT_DIR, "test_seed_run_level_hpmcounter_stats.csv")
PLOT_DIR = os.path.join(SCRIPT_DIR, "test_seed_run_level_plots")
...
def plot_run_level_dots(summary, counters):
    ...
    for counter in counters:
        ...
        plt.title(f"{counter}: per-run medians by run type (test_seed_1..3)")
        plt.legend()
        plt.tight_layout()
