"""Mean-based variant of test_seed_pre_post_attack_analysis.py.

The median-based pre/post test is deliberately blind to a brief, sharp
spike within a half - it only reports the "typical" value and ignores
outliers unless they make up more than half the samples. If an attack
(especially jump, a sudden one-time step change) causes a short burst
right around onset that settles back down, a median of the whole
post-onset half could wash that signal out entirely.

This script re-runs the exact same before/after attack-onset design as
test_seed_pre_post_attack_analysis.py, but summarizes each half of a run
with the mean instead of the median, across all 8 surviving hpmcounters
and all 20 seeds. Same purpose as before: normal acts as a negative
control - if a counter's post-attack mean is different but its normal-mode
pre/post mean is not, that's evidence specific to the attack rather than a
generic run-order/warm-up artifact.
"""
...
STATS_OUT = os.path.join(SCRIPT_DIR, "test_seed_pre_post_attack_stats_mean.csv")
...
def pre_post_means(seed_name, run_type, counters):
    path = os.path.join(CLEAN_ROOT, seed_name, f"ekf_{run_type}_hpc.csv")
    df = pd.read_csv(path)
    ...
    return rates[pre_mask].mean(), rates[post_mask].mean()


def build_pairs(seed_names, run_type, counters):
    rows = []
    for seed_name in seed_names:
        pre, post = pre_post_means(seed_name, run_type, counters)
...
def run_tests(seed_names, counters):
    ...
    for run_type in ALL_TYPES:
        ...
        for counter in counters:
            sub = pairs[pairs["counter"] == counter].dropna(subset=["pre", "post"])
            pre_vals = sub["pre"].to_numpy()
            post_vals = sub["post"].to_numpy()

            ttest_stat, p_value = ttest_rel(post_vals, pre_vals)
            ...
            rows.append({
                ...
                "mean_diff_post_minus_pre": np.mean(diffs),
...
def main():
    ...
    print(f"\nSaved mean-based pre/post attack-onset test results to {STATS_OUT}")
