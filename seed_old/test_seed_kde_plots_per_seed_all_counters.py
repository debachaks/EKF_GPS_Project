"""Per-seed KDE plots for every hpmcounter that survived cleaning: one PNG
per (counter, test_seed_N), using only that seed's rows - normal vs drift vs
jump vs replay. Unlike test_seed_kde_plots.py (which pools rows across all
seeds for a smoother density), this shows each seed's run in isolation.

Output: test_seed_kde_plots/<counter>_per_seed/<seed>_<counter>_kde.png
"""
