"""G_final: same two-stage window-diff + z-score construction as
d_final_metric.py / v_final_metric.py, but with G's statistic --
windowed mean level -- as the base signal, computed on a SINGLE sliding
window (not the two-adjacent-non-overlapping-blocks contrast used by
window_diff_metric.py's original G/g).

    G(t)       = mean(z_i for i in a single W-sample window ending at t)
    G_new_w(t) = |G(t) - G(t-1)|

    g(t) = (G_new_w(t) - mu_Gnew(t)) / sigma_Gnew(t)

        mu_Gnew(t)    = mean of G_new_w(t) across the 20 normal trials,
                        at that window position
        sigma_Gnew(t) = std of G_new_w(t) across the 20 normal trials,
                        at that window position

This makes G structurally identical to D_final/V_final: Stage 1 computes
one number per window position (here just the window's own mean, instead
of a slope-t-stat or a log-variance), Stage 2 diffs that Stage-1 sequence
across time, then Stage 3 z-scores the diff against the normal baseline.
The original window_diff_metric.py's G is a single-stage metric (the diff
of two adjacent non-overlapping blocks IS the statistic); this version
does not use that construction at all.

Same sigma-fragility guard (positions with sigma_Gnew(t) < 1e-6 excluded
from thresholding/flagging rather than clamped), same
per-trial-max-then-95th-percentile threshold, and same post-onset-only
detection rule (iter >= 150) as d_final_metric.py / v_final_metric.py.
"""
...
G_OUT_PATH = os.path.join(RESULTS_DIR, "g_final_g.csv")
GNEW_OUT_PATH = os.path.join(RESULTS_DIR, "g_final_gneww.csv")
BASELINE_OUT_PATH = os.path.join(RESULTS_DIR, "g_final_baseline.csv")
GSCORE_OUT_PATH = os.path.join(RESULTS_DIR, "g_final_gscore.csv")
THRESHOLD_OUT_PATH = os.path.join(RESULTS_DIR, "g_final_thresholds.csv")
SUMMARY_OUT_PATH = os.path.join(RESULTS_DIR, "g_final_detection_summary.csv")
