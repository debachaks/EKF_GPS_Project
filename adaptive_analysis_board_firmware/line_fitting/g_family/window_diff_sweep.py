"""Window-size sweep for the G/g metric (window_diff_metric.py: two
adjacent non-overlapping windows of length L, mean-contrasted and
z-scored against the normal-mode baseline), trying L = 5, 10, 15, 20, 25,
...
SWEEP_OUT_PATH = os.path.join(RESULTS_DIR, "window_diff_sweep.csv")
...
def run_one_window(ts, L):
    ...
    thresholds = per_run_max.groupby("counter")["M_j"].quantile(PERCENTILE / 100).reset_index(name="Qgs")
...
def main():
    ...
    for L in WINDOWS:
        print(f"Running L={L}...")
        summary = run_one_window(ts, L)
        for _, row in summary.iterrows():
            sweep_rows.append({
                "window": L, "counter": row["counter"], "mode": row["mode"],
    ...
    for mode in ["drift", "jump"]:
        print(f"\n=== {mode} ===")
        header = "Counter".ljust(14) + "".join(f"L={w}".ljust(8) for w in WINDOWS)
        print(header)
        for c in STRONG:
            row = c.ljust(14)
            for w in WINDOWS:
