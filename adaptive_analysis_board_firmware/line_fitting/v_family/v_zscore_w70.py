"""Same as v_zscore_w50.py (second-layer z-score of V, standardized against
its own per-iteration normal-mode baseline), but with W_V=70 instead of 50
-- does widening the V window further improve drift detection, or is W=50
already past its own sweet spot? Outputs go to separate _w70-suffixed
files so the W=50 results aren't touched.
"""
...
V_RAW_OUT_PATH = os.path.join(RESULTS_DIR, "v_raw_w70.csv")
V_BASELINE_OUT_PATH = os.path.join(RESULTS_DIR, "v_baseline_mu_sigma_w70.csv")
V_ZSCORE_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_w70.csv")
THRESHOLD_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_threshold_w70.csv")
FLAGGED_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_attack_flags_w70.csv")
SUMMARY_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_detection_summary_w70.csv")
...
def main():
    ...
    print("\nPer-run detection summary (z_V, W_V=70) (a run counts as detected if ANY window exceeds h_zV):")
