"""Alternate V-metric pipeline: instead of thresholding raw V = log(variance+1)
directly against a single percentile (variability_metric.py's approach),
this standardizes V itself against its own normal-mode baseline first --
a second layer of z-scoring on top of the first.

Steps:
    1. Compute V(window_end_iter) for every run (all 20 normal + jump/drift),
       using a wide window W_V=50 (compute_variation, same math as
       variability_metric.py, just applied at a much larger window).
    2. From the 20 normal runs only, build mu_V(iter) and sigma_V(iter) per
       counter -- the mean/std of V AT EACH window position across the 20
       normal baseline runs (same idea as zscore_baseline.py's mu(iter)/
       sigma(iter) for the raw counter, just one level up: this baselines
       the V STATISTIC, not the raw counter value).
    3. z_V(iter) = (V(iter) - mu_V(iter)) / sigma_V(iter), for every run.
    4. h_zV per counter = 95th percentile of {max over window positions of
       z_V, per normal run} -- same threshold-calibration convention as
       every other metric in this pipeline.
    5. Flag jump/drift runs where z_V ever exceeds h_zV.

Reads zscore_baseline.py's zscore_timeseries.csv (raw per-iteration z,
independent of any window choice).
"""
...
V_RAW_OUT_PATH = os.path.join(RESULTS_DIR, "v_raw_w50.csv")
V_BASELINE_OUT_PATH = os.path.join(RESULTS_DIR, "v_baseline_mu_sigma_w50.csv")
V_ZSCORE_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_w50.csv")
THRESHOLD_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_threshold_w50.csv")
FLAGGED_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_attack_flags_w50.csv")
SUMMARY_OUT_PATH = os.path.join(RESULTS_DIR, "v_zscore_detection_summary_w50.csv")
...
def main():
    ...
    thresholds_idx = thresholds.set_index("counter")
    ...
    print("\nPer-run detection summary (z_V, W_V=50) (a run counts as detected if ANY window exceeds h_zV):")
