...
the repo):
  ...
  - Figure 3: plots_heatmap/ml_persistence_counter_breakdown.png (~3.1in x
    2.15in bar chart, pre-onset false-trigger rate per counter -- visual
    evidence for the hpmcounter3 exclusion in Section 5.2)
  - Figure 4: plots_heatmap/ml_fusion_threshold_sweep.png
Four real figures is likely enough for a 6-page paper; the earlier
"optional bar-chart summary of Tables 2-3" placeholder has been dropped
since Figure 3 now covers that ground with new evidence rather than a
re-plot of already-tabulated numbers.
...
### 3.4 Hardware Performance Counter Instrumentation
...
### 3.5 Statistical Feature Construction
...
$$D_c(t) = \left|\frac{\hat{\beta}_c(t)}{SE(\hat{\beta}_c(t))}\right|, \quad G_c(t) = \bar{z}_c(t), \quad V_c(t) = \log\!\big(1 + s_c^2(t)\big),$$
...
where $\mu_c^X(t)$ and $\sigma_c^X(t)$ are the mean and standard deviation of $X_c^\Delta(t)$ across the 20 normal trials at window position $t$. These final $d$, $g$, $v$ scores are what the fusion methodology in Section 4 operates on.
...
With the persistence feature now fixed, we traced the remaining stress-evaluation false alarms in Table 1 to their source and found them concentrated almost entirely in one candidate counter, hpmcounter3, across all three of its statistical features, for a small subset of trials — a sustained, multi-window deviation unrelated to the anomaly being detected. Figure 3 makes this concrete: pooled across D/G/V and both attack modes, hpmcounter3's pre-onset segments trigger a false positive on 33.3% of trials, versus 1.7-2.5% for each of the other four candidate counters — roughly a 15x difference, not a marginal one. Excluding this single counter from the fused feature set (5 counters $\to$ 4: hpmcounter4/5/8/10) resolves nearly all of the remaining false alarms:

*(Figure 3: `plots_heatmap/ml_persistence_counter_breakdown.png` — bar chart, pre-onset false-trigger rate per counter (fraction of trials with a nonzero persistence value before the attack starts), pooled across D/G/V and jump+drift. hpmcounter3 (highlighted) sits far above the other four, giving direct visual evidence for the exclusion decision rather than only the aggregate before/after numbers in Table 2.)*
...
Combining the two design choices just established — the persistence feature (Section 5.1) and the 4-counter set with hpmcounter3 excluded (Section 5.2) — gives the paper's final configuration. Table 3 reports its full reliability picture: both evaluations (Section 4.4), both fusion methods, both fit exclusively on normal-operation trials.
...
- [demme2013feasibility] J. Demme et al., "On the Feasibility of Online Malware Detection with Performance Counters," ISCA 2013.
- [carna2023fight] S. Carnà et al., "Fight Hardware with Hardware: System-wide Detection and Mitigation of Side-Channel Attacks Using Performance Counters," Digital Threats: Research and Practice, 2023 (arXiv:2402.13281).

Generic placeholders still needed (broad claims, no specific source picked yet): GNSS-dependent networked-system reliability; KF/state-estimator spoofing degradation; chi-squared/NIS-based spoofing detection; adversarial spoofing engineered to evade a monitored filter.
