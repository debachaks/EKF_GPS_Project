...
the repo):
  ...
  - Figure 2: plots_heatmap/ml_persistence_test_features_jump_heatmap_column.png
...
*(Figure 2: `plots_heatmap/ml_persistence_test_features_jump_heatmap_column.png` — sized for a single IEEE column (tall/narrow aspect, small annotation font, tight margins, no outer whitespace; generated by `ml/ml_persistence_feature_heatmaps.py`). Per-seed, per-feature heatmap of persistence values, pre-onset vs. post-onset windows separated by an onset line; the pre-onset block is visually near-uniformly pale while the post-onset block lights up sharply across nearly every seed and feature, giving direct visual support for why this feature discriminates well. Optional; the numeric story is already complete from Table 1 alone.)*
