...
Table 1 lists the seven counters instrumented, their measured hardware event, and their intended causal role in this experiment (M1: FP-latency hypothesis; M2: branch/call-count hypothesis).
...
Per counter, the detection threshold is $H = Q_{95}(\{\max_t |score(t)| : \text{trial} \in \text{normal trials}\})$ — the 95th percentile of each normal trial's own peak score. A window is flagged if $|score(t)| > H$.

**Trial-level decision convention.** Every reported metric in this paper is trial-level, not window-level: a trial is either flagged or it is not, and it takes exactly **one** qualifying window to flag it — how many windows fire, or by how much, is never used as evidence of a stronger or weaker detection. This convention is applied uniformly everywhere in the paper (Table 2, the ensemble tables of Section 8.3, and the pre-onset audit of Section 8.4 all use it identically), specifically so that no result can be read as "the detector fired more/less confidently," only "the detector fired, or it did not."

**Ground truth and the pre/post-onset split.** Every trial has a ground-truth label of 0 (normal) or 1 (attack), assigned at the level of the whole 300-iteration run: normal-mode trials are label 0 throughout; jump/drift trials are label 1 by mode, but the attack itself is only physically present from iteration 150 onward — the first 150 iterations of a jump/drift trial are, by construction, indistinguishable from normal operation, since nothing has been injected yet. This paper therefore treats a jump/drift trial as covering two distinct segments: **pre-onset** (iterations $<150$, no attack present) and **post-onset** (iterations $\geq150$, attack present).

**Primary metric (Table 2): does the detector catch the attack, without crying wolf on genuinely normal trials?** A trial counts as **detected** only if a qualifying flag occurs in its post-onset segment (iteration $\geq150$); a flag in the pre-onset segment — of *any* trial, normal or attack — is never counted toward this decision. Concretely: for the 40 jump+drift trials, we ask only whether their post-onset segment ever fires (Recall = TP/(TP+FN) over these 40 trials); for the 20 normal trials, we ask only whether their (arbitrary, since no attack ever occurs) post-onset segment ever fires (False-Alarm = FP/(FP+TN) over these 20 trials). This is the standard, conservative convention: it can only ever be fooled by a *post*-onset false alarm on a normal trial, never by anything that happens earlier in any trial.

**Secondary check (Section 8.4): does the detector jump the gun on a real attack trial, before the attack has actually started?** The primary convention above has a blind spot by construction: because it only ever inspects a jump/drift trial's post-onset segment, it cannot see whether that same trial's own pre-onset segment — which, again, is genuinely still "normal" at that point — gets flagged. A pre-onset flag inside a jump/drift trial is not a *false negative* (the trial can still be separately detected later, post-onset) and it is not a *false positive* either under the primary convention (pre-onset flags are simply never inspected) — but it is exactly the same kind of event as a false alarm on a normal trial, since ground truth at that point in the run is genuinely 0. We therefore run a second, separate evaluation with its own ground truth: treat each trial's pre-onset segment as its own additional label-0 case (**Normal-pre**, **Jump-pre**, **Drift-pre**), and ask the identical one-flag-is-enough question — did this segment ever fire? — using the same threshold $H$ and the same trial-level convention as above. This is never mixed into the Recall/False-Alarm numbers of Table 2; it is reported entirely separately, in Section 8.4, precisely because Table 2's convention is silent on it by design.

## 7. Experimental Setup

20 independent seeds per mode (normal, jump, drift), 300 EKF iterations each, on a RISC-V embedded target. Detector window sizes were selected per-metric via a window-size sweep ($W \in \{5,10,15,20,25,30\}$ for D/G, $\{5,10,20,30,50\}$ for V), favoring the window size that gave the best post-onset detection rate on drift (the harder of the two attack profiles; jump recall was already saturated at 1.0 across nearly every window size tested) without over-fitting to a single seed: $W{=}10$ for D, $W{=}5$ for G and V. Five of the seven instrumented counters (hpmcounter3/4/5/8/10) produce usable (non-fully-fragile) data; hpmcounter6/7 are excluded (Section 5.2).

Detection is scored at the run level as a standard binary classification problem, using the trial-level convention and pre/post-onset ground-truth split defined in Section 6.4: ground truth is 1 for jump/drift runs' post-onset segment, 0 for normal runs (jump and drift pooled into a single "attack" class, 40 attack runs + 20 normal runs = 60 runs per counter); a run is predicted positive if it is *detected* per Section 6.4. We report Precision, Recall, F1, and False-Alarm rate (FP / (FP+TN)); the separate pre-onset check (Normal-pre/Jump-pre/Drift-pre) is reported independently in Section 8.4 and does not enter these numbers.
...
### 8.3 Ensemble voting across detectors, with persistence

Combining the three detectors' per-window flags (A=G, B=D, C=V) under three voting rules — I) A or B or C; II) at least two fire; III) A and B and C fire simultaneously at the same window — was first tried with no temporal requirement at all: a window counts as a "fire" the instant the rule is satisfied, evaluated across the whole run. This plain version inherited the same pre-onset contamination documented in Section 8.4, amplified by the OR structure of Scenario I in particular: pooled across the five usable counters, its pre-onset false-alarm rate reached 75% of jump trials and 55% of drift trials — an OR rule is only as clean as the noisiest single detector that happens to fire early.

We apply the same persistence principle used to distinguish a sustained signal from an isolated noise spike elsewhere in this pipeline: a rule only counts as satisfied if it holds for at least 3 **consecutive** windows, not a single instant. Applied to Scenarios I and II, this brought Scenario I's pre-onset false-alarm rate down to 15% (both jump and drift) with almost no loss of recall. Applied to Scenario III, however, the same filter was destructive: III's requirement that all three detectors agree at the *same* window is already rare and brief by construction, and stacking a 3-consecutive-window requirement on top of it collapsed its recall (90% $\to$ 5% jump, 50% $\to$ 10% drift, pooled). We therefore adopt a mixed final rule — Scenarios I and II require $\geq$3 consecutive windows of agreement; Scenario III keeps its original single-window definition — and report it as the final ensemble configuration below.

**Table 3: Ensemble detection rate (post-onset, out of 20 seeds), final mixed rule**

| Counter | Jump I | Jump II | Jump III | Drift I | Drift II | Drift III |
|---|---|---|---|---|---|---|
| hpmcounter3 | 20/20 | 10/20 | 14/20 | 18/20 | 8/20 | 8/20 |
| hpmcounter4 | 20/20 | 11/20 | 14/20 | 17/20 | 6/20 | 6/20 |
| hpmcounter5 | 20/20 | 14/20 | 16/20 | 18/20 | 10/20 | 5/20 |
| hpmcounter8 | 20/20 | 11/20 | 16/20 | 18/20 | 12/20 | 6/20 |
| hpmcounter10 | 20/20 | 9/20 | 14/20 | 18/20 | 7/20 | 7/20 |

Scenario I is the most accurate rule under this measure: full jump detection (20/20) for every counter and near-full drift detection (17-18/20), clearly ahead of Scenarios II and III, both of which shed substantial recall as the agreement requirement tightens — consistent with the original OR-based intuition (if any one detector reliably catches the attack, requiring the others to agree only removes true positives) once persistence has already done the work of suppressing OR's pre-onset liability.

*(Figure: pre-onset false-alarm rate vs. voting rule, one panel per counter — or the combined 1$\times$5 layout — jump and drift as separate lines; `plots_heatmap/ensemble_persistence_false_alarm_{counter}.png` / `_all_counters.png`.)* Scenario III's false-alarm rate is 0.00 for every counter, in both jump and drift — its single-window three-way-agreement condition is already strict enough that persistence was never needed for false-alarm control there, only recall was at risk. Scenario II is likewise at or near 0.00 (0.10 for hpmcounter3 jump, 0.05 for hpmcounter3 drift, 0.00 elsewhere). Scenario I — the adopted rule — carries the entire residual false-alarm cost of this ensemble: 0.15 for hpmcounter3 (both jump and drift), 0.05 for hpmcounter10 jump, and 0.00 for every other counter/attack-type pair — a small, fully disclosed cost against which Scenario I's near-perfect recall is weighed.

### 8.4 Honest accounting of false alarms: a pre-onset audit

Section 6.4 defines this audit precisely: it asks, separately from Table 2, whether each trial's *pre*-onset segment — genuinely label-0, whether the trial is normal, jump, or drift — ever produces a qualifying flag, using the identical one-flag-is-enough, trial-level convention as every other result in this paper. A pre-onset flag in a normal trial and a pre-onset flag in a jump/drift trial are the same kind of event by this definition (a label-0 segment firing); we report them side by side for exactly that reason.

Table 4 gives this per counter and per metric, as **trials-out-of-20 with $\geq1$ pre-onset flag**, for Normal-pre, Jump-pre, and Drift-pre. Two patterns stand out. First, Normal-pre is close to silent everywhere — at most 1/20 for any counter/metric — meaning a trial that never sees an attack essentially never fires early, regardless of which segment of it is inspected; this is a useful cross-check that the post-onset-only convention of Table 2 is not merely hiding pre-onset noise on normal trials specifically. Second, Jump-pre and Drift-pre are not silent to the same degree: hpmcounter3 shows the clearest pre-onset activity under every metric (D: 10/20 jump, 5/20 drift; G: 6/20 jump, 4/20 drift; V: 7/20 jump, 8/20 drift), while hpmcounter4/8/10 are close to zero throughout and hpmcounter5 sits in between. Since a jump/drift trial's pre-onset segment is, by construction, physically identical to a normal trial up to iteration 150 (no attack has been injected yet), this is not evidence that the attack "leaks backward" in time — it reflects ordinary between-seed sensor-noise variation that hpmcounter3 in particular is more sensitive to picking up (Section 9, item 6 traces this to mode-dependent noise-stream realizations in the test harness itself, not to the detector).

**Table 4: Pre-onset audit — trials (out of 20) with $\geq1$ pre-onset flag, per counter**

*D metric (W=10)*

| Counter | Normal-pre | Jump-pre | Drift-pre |
|---|---|---|---|
| hpmcounter3 | 0/20 | 10/20 | 5/20 |
| hpmcounter4 | 0/20 | 0/20 | 0/20 |
| hpmcounter5 | 0/20 | 1/20 | 0/20 |
| hpmcounter8 | 1/20 | 1/20 | 0/20 |
| hpmcounter10 | 0/20 | 0/20 | 0/20 |

*G metric (W=5)*

| Counter | Normal-pre | Jump-pre | Drift-pre |
|---|---|---|---|
| hpmcounter3 | 1/20 | 6/20 | 4/20 |
| hpmcounter4 | 1/20 | 1/20 | 1/20 |
| hpmcounter5 | 0/20 | 0/20 | 0/20 |
| hpmcounter8 | 0/20 | 0/20 | 0/20 |
| hpmcounter10 | 0/20 | 0/20 | 0/20 |
...
| Counter | Normal-pre | Jump-pre | Drift-pre |
|---|---|---|---|
| hpmcounter3 | 0/20 | 7/20 | 8/20 |
| hpmcounter4 | 0/20 | 0/20 | 0/20 |
| hpmcounter5 | 0/20 | 1/20 | 1/20 |
| hpmcounter8 | 0/20 | 1/20 | 0/20 |
| hpmcounter10 | 0/20 | 3/20 | 0/20 |

Because Section 6.4's primary convention excludes this entire segment from Precision/Recall/F1/False-Alarm as reported in Table 2, none of the above changes those numbers — this table exists purely so that the pre-onset behavior this project's own design choice deliberately does not score is nonetheless fully disclosed rather than silently discarded.
...
2. **Two of seven counters unusable.** hpmcounter6/7 carry no signal in this workload, saturated by the EKF's own steady-state computation; a different filter implementation (larger state dimension, different matrix-heavy operations) might shift which counters are informative.

3. **Threshold false-alarm/miss-rate trade-off.** The 95%-confidence ANIS threshold (Section 4.2) and 95th-percentile detection threshold (Section 6.4) are conventional choices, not uniquely optimal; tightening either would reduce false alarms at some cost to sensitivity for subtler attacks (drift consistently harder to detect than jump throughout this work).

4. **Attack model scope.** Only jump and (a specific profile of) drift spoofing were evaluated; more sophisticated, adaptive, or gradually-onset spoofing strategies designed with knowledge of this detection scheme are not evaluated and are a natural direction for future work. In particular, Section 1.1 establishes that a measurement-only attacker with white-box knowledge of `ANIS_THRESHOLD` who successfully keeps the true ANIS under it for the full attack duration evades the software ANIS check and the HPC side channel *together*, since HPC visibility is entirely mediated through the same gated branch; no such adaptive, threshold-aware attack was constructed or evaluated here, and doing so (along with measuring how much such an attack's effectiveness is itself constrained by having to stay under the gate) is future work.

5. **Mode-dependent pre-onset activation rate.** The pre-onset audit (Section 8.4, Table 4) shows pre-onset activity is not perfectly mode-independent: Normal-pre is close to silent for every counter/metric (at most 1/20), while Jump-pre/Drift-pre are non-trivial for hpmcounter3 specifically (up to 10/20). This is structurally harmless to every reported detection metric in this paper, by design: Section 6.4's primary convention excludes the entire pre-onset segment from the Precision/Recall/F1/False-Alarm numbers of Table 2, for every mode, so this asymmetry cannot affect them. We traced hpmcounter3's elevated pre-onset activity to the test harness generating slightly different sensor-noise realizations per mode even in the shared pre-onset segment (a seed-level audit found cases where jump/drift pre-onset noise streams diverge from normal's, and occasionally from each other), rather than to any backward leakage from the injected attack, which by construction does not execute before iteration 150. A fully mode-matched noise-generation process in the test harness would be a cleaner setup, and is a natural target for future work before drawing conclusions from pre-onset behavior specifically.

## 10. Conclusion

We showed that gating a Strong Tracking Filter's adaptive covariance inflation behind a formal ANIS threshold resolves a direct tension between classical tracking robustness and spoofing detectability, and that the resulting conditional branch is independently observable via RISC-V hardware performance counters. Three convergent window-diff detection statistics built over this side channel achieve F1 $\geq$ 0.96 with $\leq$5% false-alarm rates on all five usable counters, with honest accounting for both a sigma-fragility failure mode and pre-attack noise contamination. This suggests hardware-counter telemetry is a viable, filter-output-independent detection layer for GPS spoofing on embedded adaptive-filtering platforms.
